...
export async function storeUserSpace(storeName, userSpaceDetails) {
  if (!userSpaceDetails) {
    return
  }
...
export async function getUserSpaces(storeName) {
  const storedDetails = await getItemAsync(storeName)
  const parsedDetails = JSON.parse(storedDetails)
  if (!parsedDetails) {
    return {}
...
export async function modifySpaceDetails(storeName, updatedSpaceDetails) {
  if (!updatedSpaceDetails) {
    return
  }
  const storedSpaces = await getUserSpaces(storeName)
  const index = Array.prototype.findIndex.call(storedSpaces, (space) =>
    areAddressesEqual(space.address, updatedSpaceDetails.address),
  )
  if (index < 0) {
    throw new Error('Address not found in Spaces list')
  }
  storedSpaces[index] = updatedSpaceDetails
  const serialised = JSON.stringify(storedSpaces)
  await setItemAsync(storeName, serialised)
}
