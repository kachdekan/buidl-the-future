...
contract UsingPrecompiles {
    ...
    function numberValidatorsInCurrentSet() public view returns (uint256) {
        bytes memory out;
        bool success;
        (success, out) = NUMBER_VALIDATORS.staticcall(
            abi.encodePacked(uint256(block.number))
        );
        require(
            success,
            "error calling numberValidatorsInCurrentSet precompile"
        );
        return getUint256FromBytes(out, 0);
    }
    ...
    function checkProofOfPossession(
        ...
        bool success;
        (success, ) = PROOF_OF_POSSESSION.staticcall(
    ...
    function getBlockNumberFromHeader(
        ...
        bytes memory out;
        bool success;
        (success, out) = BLOCK_NUMBER_FROM_HEADER.staticcall(
    ...
    function getParentSealBitmap(
        ...
        bytes memory out;
        bool success;
        (success, out) = GET_PARENT_SEAL_BITMAP.staticcall(
            abi.encodePacked(blockNumber)
        );
    ...
    function getVerifiedSealBitmapFromHeader(
        ...
        bytes memory out;
        bool success;
        (success, out) = GET_VERIFIED_SEAL_BITMAP.staticcall(
            abi.encodePacked(header)
        );
        require(
            success,
            "error calling getVerifiedSealBitmapFromHeader precompile"
        );
        return getBytes32FromBytes(out, 0);
    }
